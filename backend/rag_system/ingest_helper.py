...
class SimpleIngestionHelper:
    ...
    def _load_file_to_documents(file_name: str, file_data: Path) -> List[Document]:
        ...
        reader_cls = SimpleIngestionHelper.SUPPORTED_EXTENSIONS.get(extension)
        
        if reader_cls is None:
            logger.debug(f"No specific reader for {extension}, using default string reader")
            # Читаем как текстовый файл
            return SimpleIngestionHelper._read_as_text(file_data)
        ...
        try:
            ...
            logger.warning(f"Reader {reader_cls.__name__} failed for {file_name}, fallback to text: {e}")
            return SimpleIngestionHelper._read_as_text(file_data)
